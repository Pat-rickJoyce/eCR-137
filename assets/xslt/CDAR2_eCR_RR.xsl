...
<!--
  ...
  Revision History:  09/19/16 Rick Geimer   Fixed some iframe issues (attribute typos). Added datauri support for nonXMLBody docs with B64 PDFs (does not work in IE or Edge as of this time, but does work in Chrome and FireFox).
  Revision History:  26/01/17 Sarah Gaunt   Update to render linkHtml tag
  Revision History:  31/01/17 Sarah Gaunt   Updates specific to Public Health Case Report: Reportability Response
  Revision History:  20/08/17 Sarah Gaunt   Updates specific to Public Health Case Report: Reportability Response to take into account ballot comments
  Revision History:  07/11/17 Sarah Gaunt   Updates specific to Public Health Case Report: Reportability Response for specific rendering needs
  ...
  <xsl:template match="n1:ClinicalDocument">
    <html>
      ...
      <body>
        ...
        <!--<h5>The boxed header information is usually handled separately by EHRs.</h5>-->
        <xsl:call-template name="informationRecipient" />
        <xsl:call-template name="eICRInformation" />
...
<!--        <hr align="left" color="teal" size="2" />-->
                ...
                <span class="td_label">
                  <xsl:choose>
                    ...
                    <xsl:when test="@typeCode = 'TRC'">
                      <xsl:text>Secondary Information Recipient:</xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
  ...
  <!-- SG 20171107: Added template for specific RR rendering wher the eICR Identifier needs to be rendered with the header info  -->
  <!-- eICRInformation -->
  <xsl:template name="eICRInformation">
    <xsl:if test="//n1:externalDocument[n1:templateId/@root='2.16.840.1.113883.10.20.15.2.3.10']">
      <table class="header_table">
        <tbody>
          <xsl:for-each select="//n1:externalDocument[n1:templateId/@root='2.16.840.1.113883.10.20.15.2.3.10']">
            <tr>
              <td class="td_header_role_name">
                <span class="td_label">
                  <xsl:text>eICR Identifier:</xsl:text>
                </span>
              </td>
              <td class="td_header_role_value">
                <xsl:for-each select="n1:id">
    ...
    <table class="header_table">
      <xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole">
        <xsl:if test="not(n1:id/@nullFlavor)">
          <!-- SG 20170131: Changed order of rows for RR -->
          <tr>
            <td class="td_header_role_name">
              <span class="td_label">
  ...
  <xsl:template match="n1:component/n1:structuredBody">
    <xsl:for-each select="n1:component/n1:section">
      <!-- SG 20170131: Changed to apply-templates -->
  ...
  <xsl:template match="n1:component/n1:nonXMLBody">
    <xsl:choose>
      ...
      <xsl:when test="n1:text/@mediaType = &quot;application/pdf&quot; and n1:text/@representation = &quot;B64&quot;">
        <iframe name="nonXMLBody" id="nonXMLBody" width="100%" height="600" src="data:application/pdf;base64,{n1:text/text()}" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="n1:section" priority="-1">
    <!-- SG: 20190327: Removed call to section-title template that was creating empty TOC link -->
  ...
  <!-- <xsl:template match="n1:section[n1:templateId[@root='2.16.840.1.113883.10.20.15.2.2.1']]" priority="1">
        <xsl:call-template name="section-title">
            <xsl:with-param name="title" select="n1:title" />
        </xsl:call-template>
        <xsl:call-template name="section-author" />
...
-->
  ...
  <!-- SG 20170125: added to render linkHtml -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <p>WARNING: non-local image found <xsl:value-of select="$image-uri"
           />. Removing. If you wish non-local images preserved please set the limit-external-images param to 'no'.</p>
        <xsl:message>WARNING: non-local image found <xsl:value-of select="$image-uri"
           />. Removing. If you wish non-local images preserved please set the limit-external-images param to 'no'.</xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="show-name">
    ...
    <xsl:choose>
      <xsl:when test="$name/n1:family">
        ...
        <!-- SG 20170131: Changed order to family, given for RR -->
